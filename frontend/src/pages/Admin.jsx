...
export default function Admin() {
  ...
  const [products, setProducts] = useState([]), [orders, setOrders] = useState([]), [slides, setSlides] = useState([]);
  ...
  async function load() {
    try {
      const [productData, orderData, slideData] = await Promise.all([api('/api/products'), api('/api/orders', { headers: auth }), api('/api/carousel')]);
      setProducts(productData); setOrders(orderData); setSlides(slideData);
  ...
  return <main className="admin"><div className="admin-top"><Brand /><button onClick={() => { localStorage.removeItem('cherie-token'); navigate('/'); }}>Sign out</button></div><p className="eyebrow">DASHBOARD</p><h1>Manage collection</h1>{notice && <p className="notice">{notice}</p>}{error && <p className="error">{error}</p>}<ProductForm editing={editingProduct} image={productImage} onImageChange={setProductImage} onUpload={file => upload(file, setProductImage)} onSave={saveProduct} onCancel={() => { setEditingProduct(null); setProductImage(''); }} /><CarouselForm editing={editingSlide} image={slideImage} onImageChange={setSlideImage} onUpload={file => upload(file, setSlideImage)} onSave={saveSlide} onCancel={() => { setEditingSlide(null); setSlideImage(''); }} slides={slides} onEdit={startEditSlide} onDelete={deleteSlide} apiImg={apiImg} /><ProductList products={products} onEdit={startEditProduct} onDelete={deleteProduct} /><OrderList orders={orders} onStatusChange={changeStatus} /></main>;
}
